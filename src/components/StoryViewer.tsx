...
//     if (item.mediaType === "image") startProgress(item.durationSec || 5);
...
export default function StoryViewer({ story }: Props) {
  ...
  // ensure session viewer exists (client-only)
  useEffect(() => {
    ensureSessionViewer();
  }, []);

  // when index changes, mark viewed and start progress for images
  useEffect(() => {
    const item = items[index];
    // mark viewed in sessionStorage (client only)
    markViewed(item.id);
    setLoaded(false);
  ...
  // pause / resume video playback on isPaused change
  useEffect(() => {
    const item = items[index];
    if (item.mediaType === "video") {
      if (!isPaused) {
  ...
  function startProgress(sec: number) {
    stopProgress();
    // store timeout id (window.setTimeout returns number)
    timerRef.current = window.setTimeout(() => next(), sec * 1000);
  }
  ...
  // Use currentTarget so we always measure the clickable container, not the inner element
      ...
      <div className="story-top">
        ...
        <div className="meta-row">
          ...
          <div className="meta-text">
            <strong>{story.user.name}</strong>
            <div className="small">
              {/* Render timestamp client-side (this component is client) */}
              {new Date(current.timestamp).toLocaleString()}
            </div>
          </div>
        </div>
